...
class Expander {
  ...
  expand() {
    ...
    for (const target of this._unexpanded.maps.targets) {
      for (const mapping of this._unexpanded.maps.byTarget(target)) {
        const expandedMap = this._expanded.maps.findByTargetAndIdentifier(target, mapping.identifier);
        if (typeof expandedMap === 'undefined') {
          this.expandMapping(mapping);
        }
      }
    }
  ...
  expandMapping(mapping) {
    ...
    const de = this._expanded.dataElements.findByIdentifier(mapping.identifier);
    if (typeof de === 'undefined') {
      this._errors.push(new MappingError(`Cannot find source data element for mapping: ${mapping.identifier.fqn}`));
      return;
    }

    let targetItem = mapping.targetItem;
    ...
    for (const baseID of de.basedOn) {
      ...
      const base = this.lookupMapping(mapping.targetSpec, baseID);
      if (typeof base !== 'undefined') {
        if (typeof targetItem === 'undefined' && typeof base.targetItem !== 'undefined') {
          ...
          this._errors.push(new MappingWarning(`${mapping.identifier.fqn} maps to ${targetItem}, but based on class ${base.identifier.fqn} maps to ${base.targetItem}`));
          continue;
        }
    ...
    if (typeof targetItem === 'undefined') {
      this._errors.push(new MappingError(`Cannot determine target item for mapping of: ${mapping.identifier.fqn}`));
      return;
    }

    // Go through this mappings rules, resolve the identifiers, and add them to the big rules list
    for (const mappingRule of mapping.rules) {
      const rule = mappingRule.clone();
      let valid = true;
      if (rule instanceof models.FieldToFieldMappingRule || rule instanceof models.FieldToURLMappingRule) {
        let currentDE = de;
        for (let i=0; i < rule.sourcePath.length; i++) {
          ...
          if (match) {
            ...
            if (i < (rule.sourcePath.length-1)) {
              currentDE = this._expanded.dataElements.findByIdentifier(match);
              if (typeof de === 'undefined') {
                this._errors.push(new MappingError(`Cannot find source data element in source path ${rule.sourcePath[i].fqn} in ${de.identifier.fqn} mapping`));
                valid = false;
                break;
              }
            }
          } else {
            this._errors.push(new MappingError(`Cannot resolve element in source path ${rule.sourcePath[i].fqn} in ${de.identifier.fqn} mapping`));
            valid = false;
            break;
          }
        }
    ...
    for (const rule of allRules) {
      const i = mergedRules.findIndex(item => {
        return rule.sourcePath && this.equalPaths(rule.sourcePath, item.sourcePath);
      });
      if (i >= 0) {
        mergedRules[i] = rule;
      } else {
  ...
  findMatchInDataElement(de, idToMatch) {
    ...
    for (const value of [de.value, ...de.fields]) {
      if (value) {
        ...
        if (match && result) {
          this._errors.push(new MappingError(`Found multiple matches for field ${idToMatch.name} in ${de.identifier.fqn} mapping`));
        } else if (match) {
          result = match;
        }
      }
  ...
  findMatchInValue(value, idToMatch) {
    if (value instanceof models.IdentifiableValue) {
      if (idToMatch.namespace && value.identifier.equals(idToMatch)) {
        return value.identifier.clone();
      } else if (!idToMatch.namespace && value.identifier.name == idToMatch.name) {
        return value.identifier.clone();
      }
  ...
  lookupMapping(targetSpec, identifier) {
    ...
    if (typeof mapping === 'undefined') {
      // We didn't find it, so look in the unexpanded mappings, then expand it (if applicable)
      const unexpanded = this._unexpanded.maps.findByTargetAndIdentifier(targetSpec, identifier);
      if (typeof unexpanded !== 'undefined') {
        return this.expandMapping(unexpanded);
      }
    }
    return mapping;
  }
  ...
  equalPaths(sourcePathA, sourcePathB) {
