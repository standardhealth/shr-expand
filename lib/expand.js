...
class Expander {
  ...
  consolidateIncludesTypeContstraint(element, value, constraint, previousConstraints) {
    ...
    if (!this.checkHasBaseType(constraint.isA, target.identifier)) {
      ...
      return constraints;
    // TODO: Check if cardinality fits into overall cardinality.
    }
    //let valueCard = value.effectiveCard
    //          else: value.constraintsFilter.card.withPath(constraint.path).constraints[0].card
    //          else: 
    
    if (target.card && !constraint.card.fitsWithinCardinalityOf(/*elementCard*/)) {
      logger.error('Cannot constrain cardinality of %s from %s to %s', targetLabel, target.card.toString(), constraint.card.toString());
      return constraints;
    }
    ...
    if (constraint.path.length == 1) {
      ...
      var finalCard = finalEl.effectiveCard;
    } else {
