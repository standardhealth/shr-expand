...
class Expander {
  ...
  consolidateConstraints(element, value) {
    ...
    for (const constraint of value.constraints) {
      ...
      if (constraint instanceof models.CardConstraint) {
        ...
        consolidateFn = this.consolidateIncludesTypeContstraint;
  ...
  consolidateIncludesTypeContstraint(element, value, constraint, previousConstraints) {

    var target = this.constraintTarget(value, constraint.path);
    var targetLabel = this.constraintTargetLabel(value, constraint.path);   

    if (target.identifier.name == "Value") {
      if (target.identifier.namespace == "unknown") {
        target = this.lookup(value.identifier).value;
      } else {
        logger.error("Abhijay fix this");
      }
    }
    ...
    // // If the constraint is actually on the target's value, update the path to explicitly include the target's value
    // let skipCheck = false;
    // if (this.constraintTarget(value, constraint.path).identifier.name == "Value") {
    //   const targetVal = this.constraintTargetValue(value, constraint.path);
    //   // If it's a choice, check if the constraint is the choice or a subtype of the choice
    //   if (targetVal instanceof models.ChoiceValue) {
    //     let isValidOption = false;
...
*/
    ...
    let constraints = previousConstraints;
    if (!this.checkHasBaseType(constraint.isA, target.identifier)) {
    ...
    if (value.constraintsFilter.card.withPath(constraint.path).constraints.length > 0) { //Explicit overriden cardinality
      var targetCard = value.constraintsFilter.card.withPath(constraint.path).constraints[0].card;
    } else if (constraint.path.length == 0 ) { //e.g. Panelmembers.Observation
      var targetCard = target.effectiveCard;
    } else if (constraint.path.length == 1) {
      var el = this.lookup(value.identifier).fields.filter(f => f.identifier.fqn == target.identifier.fqn)[0];
      var targetCard = el.effectiveCard;
    } else if (constraint.path.length > 1 ) { //e.g. SomeElement1.SomeElement2.PanelMembers.Observation
      var lastIndex = constraint.path.length - 1;
      var finalEl = this.lookup(constraint.path[lastIndex - 1]).fields.filter(f => f.identifier.fqn == target.identifier.fqn)[0];
    ...
    if (!constraint.card.fitsWithinCardinalityOf(targetCard)) {
