...
class Expander {
  ...
  consolidateConstraints(element, value) {
    ...
    for (const constraint of value.constraints) {
      ...
      if (!target) {
        ...
        consolidated = this._consolidateIncludesCodeConstraint(element, target, targetLabel, constraint, consolidated);
      }
    }
    return consolidated;
  }
  ...
  _consolidateIncludesCodeConstraint(element, target, targetLabel, constraint, previousConstraints) {
    let constraints = previousConstraints;
    if (!(target instanceof models.IdentifiableValue)) {
      this._errors.push(new MergeError(`${element.identifier.fqn}: Cannot constrain code of ${targetLabel} since it has no identifier`));
      return constraints;
