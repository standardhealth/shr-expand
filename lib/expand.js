...
class Expander {
  ...
  expandMappingForTargetAndIdentifier(target, identifier) {
    ...
    for (const rule of allRules) {
      const i = mergedRules.findIndex(item => {
        if (rule instanceof models.TargetCardinalityMappingRule && item instanceof models.TargetCardinalityMappingRule) {
          return this.equalTargetPaths(rule.targetPath, item.targetPath);
        }
  ...
  equalTargetPaths(targetPathA, targetPathB) {
    if (targetPathA.length != targetPathB.length) {
      return false;
    }
    for (let i=0; i < targetPathA.length; i++) {
      if (targetPathA[i] != targetPathB[i]) {
        return false;
      }
    }
    return true;
