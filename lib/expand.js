      ...
      for (const node of hierarchy) {
        ...
        for (const field of node.fields) {
          ...
          if (i >= 0) {
            ...
            // Still need to process the cardinality and constraints to ensure they are valid
            // (test for node == element to ensure we only report out the error on the root element)
            if (typeof f.card === 'undefined' && node == element) {
              f.setMinMax(0, 1);
              logger.error('No cardinality found for field %s in %s. Using \'0..1\' until error is resolved.  ERROR_CODE:12004', f.toString(), element.identifier.name);
            }
